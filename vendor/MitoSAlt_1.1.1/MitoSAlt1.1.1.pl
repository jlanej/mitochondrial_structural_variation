...
# PATCH (mitochondrial_structural_variation): upstream hardcodes bbmap
# reformat with -Xmx100g, which fails on any node/container without 100GB RAM.
# Make the JVM heap configurable via MITOSALT_XMX (default 4g).
my $xmx = $ENV{MITOSALT_XMX} || "4g";
...
if($nu_mt eq 'no' && $o_mt eq 'yes' && $enriched eq 'yes'){
  #REMAP ON MT GENOME
  print scalar(localtime).": Map to MT genome\n";
  system("$reformat in=$p1 in2=$p2 out=tmp_$tag.fq overwrite=true addslash=t trimreaddescription=t spaceslash=f -Xmx$xmx 2>> log/$tag.log");
  system("$lastal -Q1 -e80 -P$threads $lastindex tmp_$tag.fq|$lastsp > tmp_$tag.maf");
  system("$mfcv sam -d tmp_$tag.maf|$samtools view -@ $threads -bt $mtfaindex -|$samtools sort -@ $threads -o bam/$tag.bam -");
  system("$samtools index bam/$tag.bam");
  system("$mfcv tab tmp_$tag.maf > tab/$tag.tab");
